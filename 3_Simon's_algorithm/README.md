...
let's deal with the part of $(-1)^{x_1 \cdot j} + (-1)^{x_2 \cdot j}$ here 
...
when $j \cdot b = 0$ , there will be sum of $2^{n-1}$  (number of output for two-to-one function) probability of $(\frac{ \pm 2}{2^n})^2$, it end up with $\frac{1}{2^{n-1}}$
