...
$\frac{1}{\sqrt{N}} \sum_{k=0}^{2^n-1} e^{2 \pi i j  (\sum_{l=1}^n k_l 2^{n-l})/ 2^n} |k\rangle = \frac{1}{\sqrt{N}} \sum_{k=0}^{2^n-1} e^{2 \pi i j  \sum_{l=1}^n k_l 2^{-1}} |k\rangle$  where $\sum_{k=0}^{2^-1}$ is in the integer basis, we can express is in the sum of each bit as $\sum_{k_1=0}^1 \sum_{k_2=0}^1 \sum_{k_3=0}^1 ... \sum_{k_n=0}^1 $
