...
when $l=1$ rotation angle is $2 \pi i (j_1 2^{n-2} + j_2 2^{n-3}+ ....+ j_{n-1} 2^0+ j_n 2^{-1})$ we only care about the float part, integer is meanless in the $2 \pi$ rotation. Therefore, for l =1, rotation angle is $2 \pi i (j_n 2^{-1})$, which is $2 \pi i 0.j_n$ , where $0.j_1j_2j_3 = j_1/2+j2/4+j3/8$
